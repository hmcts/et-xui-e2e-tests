...
export default class NotificationPage extends BasePage {
  ...
  notification_tab2 =
    '//div[@class="mat-tab-label-content"]/div[@class="mat-ripple mat-tab-label mat-focus-indicator ng-star-inserted mat-tab-label-active"]/div[.="Notifications"]';
  ...
  async sendNotification(notificationType: string) {
    ...
    switch (notificationType) {
      ...
      case 'ECC':
        ...
        await this.page.getByRole('group', { name: 'Is a response to the tribunal' }).getByLabel('No').check();
        await this.page
          .getByRole('group', { name: 'What is the ECC notification?' })
          .getByLabel('Notice of Employer Contract Claim')
          .check();
        break;
      default:
