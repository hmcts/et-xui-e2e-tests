...
export default class CitizenHubLoginPage extends LoginPage {
  ...
  private readonly returnToDraftOrExistingClaimButton: Locator;
  ...
  constructor(page: Page) {
    ...
    this.returnToDraftOrExistingClaimButton = this.page.locator(`xpath=//a[normalize-space()='Return to a draft or existing claim']`);
  ...
  async assertSyaLandingPage() {
    ...
    await expect(this.returnToDraftOrExistingClaimButton).toBeVisible();
  }
  ...
  async processCitizenHubLogin(username: string, password: string){
    ...
    await this.returnToDraftOrExistingClaimButton.click();
    await this.page.waitForLoadState('load');
