...
test.describe('Upload hearing docs test', () => {
  ...
  //RET-5787
    test('for respondent - verify only future hearings are shown in options', {tag: '@demo'}, async ({ page, caseListPage, listHearingPage, et1CaseServingPage, loginPage, legalRepPage, caseDetails }) => {
        ...
        for(const number of hearingNumbers) {
          ...
          await caseDetails.checkHasBeenCreated(Events.listHearing)
        }
